...
void DrawEnemy(Enemy e) {
  DrawPoly(Vector2{e.x, e.y - e.speed}, 4, std::max(16, e.hp), 45,
           Color{255, 0, 0, 128});
  DrawPoly(Vector2{e.x, e.y}, 4, std::max(16, e.hp), 45, RED);
}
    ...
    if (!game_over && !pause) {
      ...
      if (frame_counter % frequency == 0) {
        for (int i = 0; i < stage; ++i) {
          ...
          enemies[dead_enemy].speed = 4 + (2 - log10(GetRandomValue(1, 1000)));
          enemies[dead_enemy].hp = GetRandomValue(10, 100) + difficulty * stage;
      ...
      for (auto &bullet : bullets) {
        if (bullet.alive) {
          for (auto &enemy : enemies) {
            if (enemy.alive) {
              auto hp = std::max(16, enemy.hp);
              if (CheckCollisionCircleRec(Vector2{bullet.x, bullet.y}, 2,
                                          Rectangle{enemy.x - hp, enemy.y - hp,
                                                    hp * 2.0f, hp * 2.0f})) {
